...
public:
	...
	// ── Step 1.5: runtime-dispatch wrappers ─────────────────────────────────
	...
	inline void undo(Move m) {
		// side reflects the player TO MOVE (who did NOT make the last move).
		// Dispatch on the opponent who DID make the last move.
	...
	[[nodiscard]] inline bool in_check() const {
		if (side == WHITE) {
			Square king_sq = lsb(occupancy<WHITE, KING>());
			return (attackers_of<BLACK>(king_sq, occupancy<WHITE>() | occupancy<BLACK>()) != 0);
		} else {
			Square king_sq = lsb(occupancy<BLACK, KING>());
			return (attackers_of<WHITE>(king_sq, occupancy<WHITE>() | occupancy<BLACK>()) != 0);
		}
	}

	// Declared here; implemented after MoveList (which is defined below Position).
	[[nodiscard]] inline int legal_move_count() const;

	[[nodiscard]] inline Bitboard piece_bb(Color c, PieceType pt) const {
		return pieces[Piece((c << 3) | pt)];
	}
	...
	// Midnight's set_fen() does not apply the halfmove clock FEN field.
	// Call this after set_fen() to restore the correct fifty_move_rule value.
	inline void set_fifty_move_rule(u16 value) {
		state_history.top().fifty_move_rule = value;
	}
};
...
// Step 1.5: legal_move_count() — implemented here so MoveList is fully defined.
inline int Position::legal_move_count() const {
